...
BEGIN
    ...
    SELECT 	u.username                  as username,
            ...
            ( SELECT token FROM Archive_token_MONTH as tm WHERE tm.user_id = u.id AND tm.month_start<=CURRENT_DATE AND tm.month_end>=CURRENT_DATE) as current_tokens,
            ( SELECT token FROM Archive_token_TOTAL as tt WHERE tt.user_id = u.id) as total_tokens
